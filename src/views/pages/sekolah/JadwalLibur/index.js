...
import JadwalLiburEdit from './JadwalLiburEdit';
import JadwalLiburList from './JadwalLiburList';
import oJadwalLibur from '../../../../providers/jadwalLibur';
...
export default () => {
  ...
  const refreshList = async () => {
    const resp = await oJadwalLibur.list('jadwalLibur');
    setList(resp.data)
  }
  ...
  const onDlgClose = async () => {
    if (row && row.id) {
      console.log(row.id);
      await oJadwalLibur.delete(`jadwalLibur/${row.id}`);
      refreshList();
    }
    setDlgOpen(false);
  }
  const handleFormSave = async (row) => {
    if (row.id) {
      await oJadwalLibur.update(`jadwalLibur/${row.id}`, row.fields);
    } else {
      await oJadwalLibur.insert(`jadwalLibur`, row.fields);
    }
